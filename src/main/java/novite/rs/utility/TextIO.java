...
public class TextIO {
	...
	public static boolean writeUserSelectedFile() {
		...
		while (true) {
			...
			if (selectedFile.exists()) {
				int response = JOptionPane.showConfirmDialog(null, "The file \"" + selectedFile.getName() + "\" already exists.  Do you want to replace it?", "Replace existing file?", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
	 ...
	 *            will be printed in the minimum number of spaces possible.
	 */
	 ...
	 * string if there are no characters before the end-of-line. When using
	...
	public static float getFloat() {
		...
		while (true) {
			...
			if (str == null) {
				errorMessage("Floating point number not found.", "Real number in the range " + (-Float.MAX_VALUE) + " to " + Float.MAX_VALUE);
			} else {
				try {
				...
				if (Float.isInfinite(x)) {
					errorMessage("Floating point input outside of legal range, " + str + ".", "Real number in the range " + (-Float.MAX_VALUE) + " to " + Float.MAX_VALUE);
					continue;
				}
				break;
			}
