...
import novite.rs.utility.game.npc.NPCSpawns;
...
public class Region {
	...
	public void checkLoadMap() {
		if (getLoadMapStage() == 0) {
			...
			CoresManager.slowExecutor.execute(new Runnable() {
				...
				public void run() {
					// if (Constants.DEBUG)
					// System.out.println("Load " + regionId + "...");
					try {
						loadRegionMap();
						setLoadMapStage(2);
	...
	public void clip(WorldObject object, int x, int y) {
		if (ObjectRemoval.stopBeforeSpawning(object, object.getX(), object.getY(), object.getPlane())) {
			unclip(object, x, y);
			return;
		}
		if (map == null) {
	...
	public void spawnObject(WorldObject object, int plane, int localX, int localY, boolean original) {
		/*
		 * if (ObjectRemoval.stopBeforeSpawning(object, object.getX(),
		 * object.getY(), object.getPlane())) { unclip(object, localX, localY);
		 * return; }
		 */
		if (objects == null) {
	...
	public void loadRegionMap() {
		...
		byte[] landContainerData = landArchiveId == -1 ? null : Cache.STORE.getIndexes()[5].getFile(landArchiveId, 0, MapXTEA.getKey(regionId));
		...
		if (landContainerData == null && landArchiveId != -1 && MapXTEA.getKey(regionId) != null) {
	...
	public FloorItem getFloorItem(int id, WorldTile tile, Player player) {
		...
		for (FloorItem item : floorItems) {
			if ((item.isInvisible()) && (item.hasOwner() && !player.getUsername().equalsIgnoreCase(item.getOwner().getUsername()))) {
				continue;
			}
