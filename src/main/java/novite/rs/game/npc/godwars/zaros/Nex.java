...
public class Nex extends NPC {
	 ...
	 *
	 */
	private static final long serialVersionUID = -286854002467712281L;
	...
	public void transformNex() {
		if (getId() == 15776) {
			...
			transformInto(getId() == 13449 ? 13447 : getId() + 1);
		}
	}
	...
	public ArrayList<Entity> calculatePossibleTargets(WorldTile current, WorldTile position, boolean northSouth) {
		...
		for (Entity e : getPossibleTargets()) {
			if (e.inArea(current.getX(), current.getY(), position.getX() + (northSouth ? 2 : 0), position.getY() + (!northSouth ? 2 : 0)) || e.inArea(position.getX(), position.getY(), current.getX() + (northSouth ? 2 : 0), current.getY() + (!northSouth ? 2 : 0))) {
				list.add(e);
			}
		}
		return list;
