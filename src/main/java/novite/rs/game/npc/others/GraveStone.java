...
public class GraveStone extends NPC {// 652 - gravestone selection interface
	...
	public void addLeftTime(boolean clean) {
		if (clean) {
			boolean untradeablesExisted = false;
			for (FloorItem item : floorItems) {
				if (item == null)
					continue;
				if (!ItemConstants.isTradeable(item)) {
					if (getRegion().forceGetFloorItems() == null || !getRegion().forceGetFloorItems().contains(item)) {
						continue;
					}
					getRegion().forceGetFloorItems().remove(item);
					if (getPlayer() != null) {
						if (getPlayer().getMapRegionsIds().contains(getRegionId()) && getPlayer().getPlane() == item.getTile().getPlane()) {
							getPlayer().getPackets().sendRemoveGroundItem(item);
						}
						getPlayer().getBank().addItem(item.getId(), item.getAmount(), true);
					}
					untradeablesExisted = true;
					continue;
				} else {
					World.turnPublic(item, 60);
				}
			}
			if (untradeablesExisted) {
				if (getPlayer() != null) {
					getPlayer().sendMessage("<col=" + ChatColors.MAROON + ">All of the untradeables in your gravestone were banked.</col>");
				}
			}
		} else {
	...
	public void decrementGrave(int stage, String message) {
		...
		if (stage == -1) {
			...
			transformInto(getNPCId(graveStone) + stage);
	}
	...
	public void sendGraveInscription(Player reader) {
		...
		if (ticks <= 50) // if the grave is almost broken
			reader.getPackets().sendIComponentText(GRAVE_STONE_INTERFACE, 23, "The inscription is too unclear to read.");
		else if (reader == getPlayer())
			reader.getPackets().sendIComponentText(GRAVE_STONE_INTERFACE, 23, "It looks like it'll survive another " + (ticks / 100) + " minutes. Isn't there something a bit odd about reading your own gravestone?");
		else
			reader.getPackets().sendIComponentText(GRAVE_STONE_INTERFACE, 23, inscription);
	}
