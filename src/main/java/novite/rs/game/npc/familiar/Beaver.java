...
public class Beaver extends Familiar {
	...
	public boolean submitSpecial(Object context) {
		/*
		 * Player player = (Player) context; objectLoop : for (WorldObject
		 * object : World.getRegion(player.getRegionId()).getAllObjects()) { if
		 ...
		 * objectDef = object.getDefinitions();
		 * switch(objectDef.name.toLowerCase()) { case "tree": if
		 * (objectDef.containsOption(0, "Chop down"))
		 * player.getActionManager().setAction(new Woodcutting(object,
		 * TreeDefinitions.NORMAL, true)); break objectLoop; case "evergreen":
		 * if (objectDef.containsOption(0, "Chop down"))
		 * player.getActionManager().setAction(new Woodcutting(object,
		 * TreeDefinitions.EVERGREEN, true)); break objectLoop; case
		 * "dead tree": if (objectDef.containsOption(0, "Chop down"))
		 * player.getActionManager().setAction(new Woodcutting(object,
		 ...
		 * (objectDef.containsOption(0, "Chop down"))
		 * player.getActionManager().setAction(new Woodcutting(object,
		 * TreeDefinitions.WILLOW, true)); break objectLoop; case "maple tree":
		 * if (objectDef.containsOption(0, "Chop down"))
		 * player.getActionManager().setAction(new Woodcutting(object,
		 * TreeDefinitions.MAPLE, true)); break objectLoop; case "ivy": if
		 * (objectDef.containsOption(0, "Chop"))
		 * player.getActionManager().setAction(new Woodcutting(object,
		 * TreeDefinitions.IVY, true)); break objectLoop; case "yew": if
		 * (objectDef.containsOption(0, "Chop down"))
		 * player.getActionManager().setAction(new Woodcutting(object,
		 * TreeDefinitions.YEW, true)); break objectLoop; case "magic tree": if
		 * (objectDef.containsOption(0, "Chop down"))
		 * player.getActionManager().setAction(new Woodcutting(object,
		 * TreeDefinitions.MAGIC, true)); break objectLoop; case
		 * "cursed magic tree": if (objectDef.containsOption(0, "Chop down"))
		 * player.getActionManager().setAction(new Woodcutting(object,
		 * TreeDefinitions.CURSED_MAGIC, true)); break objectLoop; } } return
		 * true; } System.out.println("FALSE");
		 * player.getPackets().sendGameMessage
		 * ("Your beaver cannot seem to find any trees around your location.");
		 * return false;
		 */
		return false;
	}
