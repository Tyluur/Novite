...
public class Voidshifter extends Familiar {
	...
	public Voidshifter(Player owner, Pouches pouch, WorldTile tile, int mapAreaNameHash, boolean canBeAttackFromOutOfArea) {
	...
	public void processNPC() {
		...
		if (getOwner().getHitpoints() <= (getOwner().getMaxHitpoints() * .1) && !getOwner().isDead()) {
			...
			Magic.sendTeleportSpell(getOwner(), 14388, -1, 1503, 1502, 0, 0, new WorldTile(2662, 2649, 0), 3, true, Magic.OBJECT_TELEPORT);
			sendDeath(getOwner());
			return;
		}
	}
