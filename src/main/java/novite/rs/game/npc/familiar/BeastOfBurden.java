...
public class BeastOfBurden implements Serializable {
	...
	public void addItem(int slot, int amount) {
		...
		if (canDepositOnly) {
			...
			return;
		} else if (!ItemConstants.isTradeable(item.getId()) || item.getId() == 4049 || (familiar.canStoreEssOnly() && item.getId() != 1436 && item.getId() != 7936) || (item.getDefinitions().getValue() * item.getAmount()) > 50000) {
