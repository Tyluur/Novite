package novite.rs.game.minigames;
...
import novite.rs.game.npc.others.CastleWarBarricade;
import novite.rs.game.player.Equipment;
import novite.rs.game.player.Player;
import novite.rs.game.player.content.Foods.Food;
import novite.rs.game.player.controlers.impl.castlewars.CastleWarsPlaying;
import novite.rs.game.player.dialogues.CastleWarsScoreboard;
...
public final class CastleWars {
	...
	private static final int playersNeededToStart = 2;
	...
	public static void joinPortal(Player player, int team) {
		...
		if (player.containsOneItem(Boolean.FALSE, 4037, 4039)) {
		...
		if (playingGame == null && waiting[team].size() >= playersNeededToStart) {
			// 9players to
			// start
			createPlayingGame();
		} else {
	...
	public static void refreshTimeLeft(Player player) {
		player.getVarsManager().sendVar(380, playingGame == null ? 0 : playingGame.minutesLeft - (player.getControllerManager().getController() instanceof CastleWarsPlaying ? 5 : 0));
	}
	...
	public static void handleInterfaces(Player player, int interfaceId, int componentId, int packetId) {
		if (interfaceId == 60) {
			if (componentId != 1 && componentId != 5 && componentId != 9) {
				interactCastleWarsTicketExchange(player, componentId, packetId == DefaultGameDecoder.ACTION_BUTTON1_PACKET);
			}
		}
	}
	...
	public static void interactCastleWarsTicketExchange(Player player, int componentId, boolean buy) {
		Integer id = rewards.get(componentId);
		if (id == null) {
			return;
		}
		if (buy) {
