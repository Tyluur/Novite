	...
	public boolean canPlayerOption1(final Player target) {
		...
		player.setRouteEvent(new RouteEvent(target, new Runnable() {
			...
			public void run() {
				...
				if (target == null || target.hasFinished() || !target.withinDistance(player, 14) || !(target.getControllerManager().getController() instanceof RequestController)) {
	...
	public boolean processObjectClick1(WorldObject object) {
		switch (object.getId()) {
			...
			case 38699: // Risk FFA portal.
				...
				player.getControllerManager().startController("clan_wars_ffa", true);
				return false;
		}
		return true;
	}
