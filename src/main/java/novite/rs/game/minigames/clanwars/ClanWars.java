...
public final class ClanWars implements Serializable {
		...
		if (other != null && (Boolean) other.getTemporaryAttributtes().get("accepted_war_terms") == Boolean.TRUE) {
			CoresManager.slowExecutor.submit(new Runnable() {
				...
				public void run() {
					...
					RegionBuilder.copyAllPlanesMap(areaType.getSouthWestTile().getChunkX(), areaType.getSouthWestTile().getChunkY(), newCoords[0], newCoords[1], width, height);
	...
	public static void enter(Player p) {
		...
		if (hasWar) {
			...
			p.getControllerManager().startController("clan_war", c);
			CoresManager.slowExecutor.submit(new Runnable() {
				@Override
				public void run() {
