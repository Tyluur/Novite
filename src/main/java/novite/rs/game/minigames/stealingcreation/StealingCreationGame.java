...
public class StealingCreationGame {
	...
	public void sendItemToBase(Player player, Item item, boolean team, boolean BOB, boolean clicked) {
		...
		objectLoop:
			for (WorldObject object : World.getRegion(tile.getRegionId()).getSpawnedObjects()) {
		...
		synchronized (logicLock) {
			...
			for (final Player player : blueTeam) {
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					@Override
					public void run() {
						player.getControllerManager().startController("StealingCreationGame", game, false);
			...
			for (final Player player : redTeam) {
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					@Override
					public void run() {
						player.getControllerManager().startController("StealingCreationGame", game, true);
