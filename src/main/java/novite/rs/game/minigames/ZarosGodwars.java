...
public class ZarosGodwars {
	...
	private static final Animation START = new Animation(6355), SECOND_START = new Animation(6987), THIRD_START = new Animation(6987);
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (stage == 0) {
					...
					nexMinion.setNextAnimation(new Animation(6987));
	...
	public static void sendBeginningAction(NexPhase phase) {
		...
		if (nexMinion != null) {
			...
			nexMinion.setNextFaceEntity(nex);
			nexMinion.setCantInteract(false);
			nex.setNextFaceEntity(nexMinion);
		}
	}
