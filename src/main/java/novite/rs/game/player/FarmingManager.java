...
public class FarmingManager implements Serializable {
	...
	private static final int REGENERATION_CONSTANT = Constants.DEBUG ? 5000 : 120000; // Twelve
	// minutes
	private static final int ALLOTMENT = 0, TREES = 1, HOPS = 2, FLOWERS = 3, FRUIT_TREES = 4, BUSHES = 5, HERBS = 6, COMPOST = 7, RAKE = 5341, EMPTY_BUCKET = 1925;
	private static final String[] PATCH_NAMES = { "allotment", "tree", "hops", "flower", "fruit tree", "bush", "herb" };
	private static final int[][] HARVEST_AMOUNTS = { { 3, 53 }, { 1, 1 }, { 3, 41 }, { 1, 3 }, { 3, 5 }, { 3, 5 }, { 3, 18 } };
	...
	private void collectTreeProducts(final FarmingSpot spot, final TreeDefinitions definitions) {
		player.getActionManager().setAction(new Action() {
			...
			@Override
			public int processWithDelay(Player player) {
				Woodcutting.addLog(definitions, false, player);
	...
	private class FarmingSpot implements Serializable {
		...
		public void renewCycle() {
			...
			if (productInfo != null) {
				cycleTime += (stage == 0 || Constants.DEBUG) ? 5000 : constant * productInfo.cycleTime;
			} else {
