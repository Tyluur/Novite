...
import novite.rs.game.player.content.achievements.impl.HerbloreHardAchievement;
...
public class Herblore extends Action {
	...
	public enum Ingredients {
		...
		GUAM(249, new short[] { Herblore.VIAL, Herblore.SWAMP_TAR }, new short[] { 91 }, new byte[] { 0, 19 }, new double[] { 0, 30 }),

		FELLSTALK(21624, new short[] { Herblore.VIAL }, new short[] { 21628 }, new byte[] { 94 }, new double[] { 0 }),
		...
		AVANTOE(261, new short[] { Herblore.VIAL, 2436, 145, 147, 149 }, new short[] { 103, 15308, 15309, 15310, 15311 }, new byte[] { 0, 88, 88, 88, 88 }, new double[] { 0, 220, 220, 220, 220 }),
		...
		LANTADYME(2481, new short[] { Herblore.VIAL, 2442, 163, 165, 167 }, new short[] { 2483, 15316, 15317, 15318, 15319 }, new byte[] { 0, 90, 90, 90, 90 }, new double[] { 0, 240, 240, 240, 240 }),

		DWARF_WEED(267, new short[] { Herblore.VIAL, 2440, 157, 159, 161 }, new short[] { 109, 15312, 15313, 15314, 15315 }, new byte[] { 0, 89, 89, 89, 89 }, new double[] { 0, 230, 230, 230, 230 }),
	...
	public boolean process(Player player) {
		if (ingredients == Ingredients.TORSTOL && otherItem.getId() != VIAL) {
			int[] items = new int[] { 15309, 15313, 15317, 15321, 15325 };
			int invalidItem = -1;
			for (int item : items) {
			...
			if (invalidItem != -1) {
				player.sendMessage("You do not have a " + ItemDefinitions.getItemDefinitions(invalidItem).name + " in your inventory.");
				return false;
			}
		}
		/**
		 * Temporary fix for stackable items TODO revisit
		 */
		if (node.getId() == 12539 && otherItem.getId() == 169) {
			if (!player.getInventory().containsItem(otherItem.getId(), otherItem.getAmount())) {
				return false;
			}
			if (!player.getInventory().contains(node.getId())) {
				return false;
			}
			return true;
		}
	...
	public int processWithDelay(Player player) {
		...
		if (otherItem.getId() == VIAL || otherItem.getId() == COCONUT_MILK || node.getId() == VIAL || node.getId() == COCONUT_MILK) {
			player.getPackets().sendGameMessage("You add the " + node.getDefinitions().getName().toLowerCase().replace("clean", "").trim() + " into the vial of " + (otherItem.getId() == VIAL ? "water." : "milk."));
		} else if (otherItem.getId() == SWAMP_TAR || node.getId() == SWAMP_TAR) {
			player.getPackets().sendGameMessage("You add the " + node.getDefinitions().getName().toLowerCase().replace("clean ", "").trim() + " on the swamp tar.", true);
		} else if (otherItem.getId() == PESTLE_AND_MORTAR || node.getId() == PESTLE_AND_MORTAR) {
			player.getPackets().sendGameMessage("You crush the " + node.getDefinitions().getName().toLowerCase().trim() + " with your pestle and mortar.", true);
			...
			player.getPackets().sendGameMessage("You mix the " + node.getDefinitions().getName().toLowerCase().trim() + " into your potion.", true);
		}
		if (node.getId() == PESTLE_AND_MORTAR) {
			player.getInventory().removeItems(new Item(otherItem.getId(), 1), rawIngredient == null ? new Item(node.getId(), 1) : null);
		} else {
			player.getInventory().removeItems(new Item(node.getId(), 1), rawIngredient == null ? new Item(otherItem.getId(), 1) : null);
		}
		Item item = rawIngredient != null ? rawIngredient.getCrushedItem() : new Item(ingredients.getRewards()[slot], 1);
		player.getInventory().addItem(item);
		if (item.getId() == 139) {
			player.getAchievementManager().notifyUpdate(HerbloreHardAchievement.class);
		}
