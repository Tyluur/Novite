package novite.rs.game.player.actions.runecrafting;
...
import novite.rs.game.player.actions.Woodcutting.HatchetDefinitions;
import novite.rs.game.player.actions.mining.MiningBase;
import novite.rs.game.player.actions.mining.MiningBase.PickAxeDefinitions;
...
public class AbbysObsticals {
	...
	protected static final int[][] ABYSS_TELEPORT_OUTER = { { 3059, 4817 }, { 3062, 4812 }, { 3052, 4810 }, { 3041, 4807 }, { 3035, 4811 }, { 3030, 4808 }, { 3026, 4810 }, { 3021, 4811 }, { 3015, 4810 }, { 3020, 4818 }, { 3018, 4819 }, { 3016, 4824 }, { 3013, 4827 }, { 3017, 4828 }, { 3015, 4837 }, { 3017, 4843 }, { 3014, 4849 }, { 3021, 4847 }, { 3022, 4852 }, { 3027, 4849 }, { 3031, 4856 }, { 3035, 4854 }, { 3043, 4855 }, { 3045, 4852 }, { 3050, 4857 }, { 3054, 4855 }, { 3055, 4848 }, { 3060, 4848 }, { 3059, 4844 }, { 3065, 4841 }, { 3061, 4836 }, { 3063, 4832 }, { 3064, 4828 }, { 3060, 4824 }, { 3063, 4821 }, { 3041, 4808 }, { 3030, 4810 }, { 3018, 4816 }, { 3015, 4829 }, { 3017, 4840 }, { 3020, 4849 }, { 3031, 4855 }, { 3020, 4854 }, { 3035, 4855 }, { 3047, 4854 }, { 3060, 4846 }, { 3062, 4836 }, { 3060, 4828 }, { 3063, 4820 }, { 3028, 4806 }, };
	...
	public static void clearRocks(final Player player, final WorldObject object) {
		...
		if (defintions == null) {
			player.getPackets().sendGameMessage("You need a usable pickaxe in order to clear this obstical.");
			return;
		}
		player.lock();
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (ticks == 1)
					...
					player.setNextAnimation(new Animation(defintions.getAnimationId()));
				} else if (ticks == 4) {
	...
	public static void clearTendrills(final Player player, final WorldObject object, final WorldTile tile) {
		...
		if (defintions == null) {
			player.getPackets().sendGameMessage("You need a usable hatchet in order to clear this obstical.");
			return;
		}
		player.lock();
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (ticks == 1)
					...
					player.setNextAnimation(new Animation(defintions.getEmoteId()));
				} else if (ticks == 3) {
	...
	public static void clearEyes(final Player player, final WorldObject object, final WorldTile tile) {
		player.lock();
		WorldTasksManager.schedule(new WorldTask() {
			int ticks = 0;
	...
	public static void clearGap(final Player player, final WorldObject object, final WorldTile tile, final boolean quick) {
		player.lock();
		WorldTasksManager.schedule(new WorldTask() {
			int ticks = 0;
				...
				if (ticks == 1)
					...
					if (!quick) {
						if (!isSuccessFul(player, Skills.AGILITY)) {
							player.getPackets().sendGameMessage("You cannot seem to slip through the gap.");
							player.unlock();
							player.setNextAnimation(new Animation(-1));
							stop();
							return;
	...
	public static void burnGout(final Player player, final WorldObject object, final WorldTile tile) {
		if (!player.getInventory().containsItem(590, 1)) {
			player.getPackets().sendGameMessage("You need a tinderbox in order to burn the boil.");
			return;
		}
		player.lock();
		WorldTasksManager.schedule(new WorldTask() {
	...
	private static void demolishObstical(int objectId, WorldObject object) {
