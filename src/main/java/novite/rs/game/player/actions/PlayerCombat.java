...
public class PlayerCombat extends Action {
	...
	public int processWithDelay(Player player) {
		...
		if (spellId > 0) {
			...
			if (isRanging == 0) {
				return (int) (meleeAttack(player) * multiplier) + this.delay;
			} else if (isRanging == 1) {
				...
				return (int) (rangeAttack(player) * multiplier) + this.delay;
			}
		}
	}
	...
	private void addAttackedByDelay(Entity player) {
		...
		target.setLastTimeHit(System.currentTimeMillis());
	}
	...
	public int mageAttack(final Player player, int spellId, boolean autocast) {
		...
		if (player.getCombatDefinitions().getSpellBook() == 192) {
			...
			if (!player.getQuestManager().completedQuest(QuestManager.getQuest(Desert_Treasure.class).getName())) {
			...
			case 36: // Miasmic rush
				...
				if (target instanceof Player) {
					target.player().getPackets().sendGameMessage("You feel slowed down.");
				}
	...
	private int getMagicMaxHit(Player player, int baseDamage) {
		...
		if (player.getEquipment().getHatId() == 13263) {
			EA *= 0.15;
		}
		double ED = 0, DB;
		if (target instanceof Player) {
	...
	private int rangeAttack(final Player player) {
		...
		if (player.getCombatDefinitions().isUsingSpecialAttack()) {
			...
			case 18332: // Magic longbow (sighted)
				...
				player.setNextGraphics(new Graphics(256, 0, 100));

				World.sendProjectile(player, target, 249, 50, 40, 30, 10, 0, 0);
				World.sendProjectile(player, target, 249, 50, 40, 30, 40, 0, 0);
			...
			if (weaponId != -1) {
				...
				if (weaponName.contains("throwing axe") || weaponName.contains("knife") || weaponName.contains("dart") || weaponName.contains("javelin") || weaponName.contains("thrownaxe")) {
					...
					player.setNextAnimation(new Animation(getWeaponAttackEmote(weaponId, attackStyle)));
					World.sendProjectile(player, target, 442, 41, 16, 25, 30, 16, 0);
					...
					World.sendProjectile(player, target, 909, 38, 36, 41, 32, 5, 0);
					attackTarget(getMultiAttackTargets(player, 3, Integer.MAX_VALUE), new MultiAttack() {

						private boolean nextTarget;

						@Override
						public boolean attack() {
							target.setNextGraphics(new Graphics(86, 0, 96));
							delayHit(1, weaponId, attackStyle, getRangeHit(player, getRandomMaxHit(player, weaponId, attackStyle, true)));
							WorldTasksManager.schedule(new WorldTask() {
								@Override
								public void run() {
					...
					if (Utils.getRandom(player.getSkills().getLevel(Skills.FIREMAKING) << 1) == 0) {
						...
						player.sendMessage("Oh no, your hand cannon has exploded!");
						return combatDelay;
					} else {
						player.setNextGraphics(new Graphics(2138));
					...
					player.setNextAnimation(new Animation(getWeaponAttackEmote(weaponId, attackStyle)));
					World.sendProjectile(player, target, 249, 50, 40, 30, 30, 0, 0);
					int hit = getRandomMaxHit(player, weaponId, attackStyle, true);
					delayHit(2, weaponId, attackStyle, getRangeHit(player, hit));
	...
	private int meleeAttack(final Player player) {
		...
		if (player.getCombatDefinitions().isUsingSpecialAttack()) {
			...
			case 21371:
				...
				player.setNextGraphics(new Graphics(476, 0, 4));
				final int wId = weaponId;
				final int aS = attackStyle;
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {
						if (count-- <= 0) {
							...
							delayNormalHit(wId, aS, getMeleeHit(player, getRandomMaxHit(player, wId, aS, false, true, 0.33, true)));
							target.setNextGraphics(new Graphics(478, 0, 4));
						}
					}
				}, 0, 1);
				break;
			...
			case 23679:
				...
				delayNormalHit(weaponId, attackStyle, new Hit[] { getMeleeHit(player, getRandomMaxHit(player, weaponId, attackStyle, false, true, 1.5D, true)) });
				break;
			case 13899: // vls
			case 13901:
			...
			case 23695:
				...
				int hit = getRandomMaxHit(player, weaponId, attackStyle, false, true, 1.25, true);
				if (hit > 0) {
			...
			case 4587: // dragon sci
				...
				if (target instanceof Player) {
					Player p2 = (Player) target;
					if (hit1.getDamage() > 0) {
						p2.getTemporaryAttributtes().put("PrayerBlocked", 5000 + Utils.currentTimeMillis());
						int[] modern = { 16, 17, 18, 19 };
						int[] ancients = { 6, 7, 8, 9 };
						if (p2.getPrayer().isAncientCurses()) {
							for (int ancient : ancients) {
								p2.getPrayer().closePrayers(ancient);
							}
						} else {
							for (int element : modern) {
								p2.getPrayer().closePrayers(element);
							}
						}
			...
			if (fullArmourSetEquipped(player, "Guthan's") && (Utils.percentageChance(25) || !Constants.isVPS)) {
				player.setNextGraphics(new Graphics(398));
				player.heal(hit.getDamage());
			}
	...
	public int getRandomMaxHit(Player player, int weaponId, int attackStyle, boolean ranging, boolean defenceAffects, double specMultiplier, boolean usingSpec) {
		...
		if (defenceAffects) {
			double att = (player.getSkills().getLevel(ranging ? 4 : 0) / 2) + player.getCombatDefinitions().getBonuses()[ranging ? 4 : CombatDefinitions.getMeleeBonusStyle(weaponId, attackStyle)];
			if (weaponId == -2) {
				att += 82;
			}
			...
			if (ranging && player.getEquipment().getHatId() == 13263) {
				att *= 0.15;
			}
			double def = 0;
			if (target instanceof Player) {
				Player p2 = (Player) target;
				def = (p2.getSkills().getLevel(Skills.DEFENCE) / 2) + p2.getCombatDefinitions().getBonuses()[ranging ? 9 : CombatDefinitions.getMeleeDefenceBonus(CombatDefinitions.getMeleeBonusStyle(weaponId, attackStyle))];
	...
	public final int getMaxHit(Player player, int weaponId, int attackStyle, boolean ranging, boolean usingSpec, double specMultiplier) {
		if (!ranging) {
			...
			if ((hatId >= 8901 && hatId <= 8922) || Slayer.hasSlayerHelmet(player)) {
				if (player.getSlayerTask() != null && target != null && target.isNPC()) {
					if (player.getSlayerTask().getName().equalsIgnoreCase(((NPC) target).getName())) {
						otherBonus += 0.125;
					}
				}
			}
			...
			boolean bNeck = player.getEquipment().getAmuletId() == 11128 && (player.getEquipment().getWeaponId() == 6528 || player.getEquipment().getWeaponId() == 6527 || player.getEquipment().getWeaponId() == 6523 || player.getEquipment().getWeaponId() == 6526);
			double max = Math.floor(baseDamage * specMultiplier * otherBonus);
			if (bNeck) {
				max = max * 1.26;
			}
	...
	public static final boolean fullArmourSetEquipped(Player player, String name) {
		int helmId = player.getEquipment().getHatId();
		int chestId = player.getEquipment().getChestId();
		int legsId = player.getEquipment().getLegsId();
		int weaponId = player.getEquipment().getWeaponId();
		...
		return ItemDefinitions.getItemDefinitions(helmId).getName().contains(name) && ItemDefinitions.getItemDefinitions(chestId).getName().contains(name) && ItemDefinitions.getItemDefinitions(legsId).getName().contains(name) && ItemDefinitions.getItemDefinitions(weaponId).getName().contains(name);
	}
		...
		for (Hit hit : hits) {
			...
			if (target instanceof Player) {
				...
				if (player.getPrayer().usingPrayer(1, 18)) {
					p2.handleSoulSplit(player, target, hit);
				}
			}
			...
			if (hit.getLook() == HitLook.RANGE_DAMAGE || hit.getLook() == HitLook.MELEE_DAMAGE) {
				double combatXp = damage;/* damage / 2.5; */
				if (combatXp > 0) {
					player.getAuraManager().checkSuccefulHits(hit.getDamage());
					if (hit.getLook() == HitLook.RANGE_DAMAGE) {
						if (attackStyle == 2) {
							player.getSkills().addXp(Skills.RANGE, combatXp / 2, target.isPlayer());
							player.getSkills().addXp(Skills.DEFENCE, combatXp / 2, target.isPlayer());
						} else {
							player.getSkills().addXp(Skills.RANGE, combatXp, target.isPlayer());
						}
					} else {
						int xpStyle = CombatDefinitions.getXpStyle(weaponId, attackStyle);
						if (xpStyle != CombatDefinitions.SHARED) {
							player.getSkills().addXp(xpStyle, combatXp, target.isPlayer());
						} else {
							player.getSkills().addXp(Skills.ATTACK, combatXp / 3, target.isPlayer());
							player.getSkills().addXp(Skills.STRENGTH, combatXp / 3, target.isPlayer());
							player.getSkills().addXp(Skills.DEFENCE, combatXp / 3, target.isPlayer());
						}
					}
					double hpXp = damage / 7.5;
					if (hpXp > 0) {
						player.getSkills().addXp(Skills.HITPOINTS, hpXp, target.isPlayer());
					}
				}
				...
				if (combatXp > 0) {
					player.getAuraManager().checkSuccefulHits(hit.getDamage());
					if (player.getCombatDefinitions().isDefensiveCasting() || (hasPolyporeStaff(player) && player.getCombatDefinitions().getAttackStyle() == 1)) {
						int defenceXp = (int) (damage / 7.5);
						if (defenceXp > 0) {
							combatXp -= defenceXp;
							player.getSkills().addXp(Skills.DEFENCE, defenceXp / 7.5, target.isPlayer());
						}
					}
					player.getSkills().addXp(Skills.MAGIC, combatXp, target.isPlayer());
					double hpXp = damage / 7.5;
					if (hpXp > 0) {
						player.getSkills().addXp(Skills.HITPOINTS, hpXp, target.isPlayer());
					}
	...
	public static int getMeleeCombatDelay(Player player, int weaponId) {
		if (weaponId != -1) {
			...
			// Interval 2.4
			if (weaponName.equals("zamorakian spear") || weaponName.equals("korasi's sword") || weaponName.equals("saradomin sword")) {
				return 3;
			}
			// Interval 3.6
	...
	private boolean checkAll(Player player) {
		if (target.isNPC() && !target.npc().getDefinitions().hasAttackOption()) {
			player.getPackets().sendGameMessage("You can't attack this npc.");
			return false;
		}
		if (target.isNPC() && !Slayer.canDamage(player, target.npc())) {
			return false;
		}
		if (player.isDead() || player.hasFinished() || target.isDead() || target.hasFinished()) {
			return false;
		}
		if (target instanceof NPC && ((NPC) target).getName().equalsIgnoreCase("Portal")) {
			return true;
		}
		...
		if (!(target instanceof NPC && ((NPC) target).isForceMultiAttacked())) {
			if (!target.isAtMultiArea() || !player.isAtMultiArea()) {
				if (!(target instanceof Glacyte)) {
					...
					Glacyte glacyte = (Glacyte) target;
					if (!glacyte.correctName(player.getUsername())) {
						player.sendMessage("You are not allowed to attack this.");
						return false;
					}
				}
			}
		...
		if (player.getFreezeDelay() >= Utils.currentTimeMillis()) {
			if (Utils.colides(player.getX(), player.getY(), size, target.getX(), target.getY(), size))// under-target
				return false;
			if (isRanging == 0 && target.getSize() == 1 && player.getCombatDefinitions().getSpellId() <= 0 && !hasPolyporeStaff(player) && Math.abs(player.getX() - target.getX()) == 1 && Math.abs(player.getY() - target.getY()) == 1 && !target.hasWalkSteps()) // diagonal
				return false;
			return true;
		}
		if (Utils.colides(player.getX(), player.getY(), size, target.getX(), target.getY(), size) && !target.hasWalkSteps()) {
			...
			return true;
		} else if ((isRanging == 0) && target.getSize() == 1 && player.getCombatDefinitions().getSpellId() <= 0 && !hasPolyporeStaff(player) && Math.abs(player.getX() - target.getX()) == 1 && Math.abs(player.getY() - target.getY()) == 1 && !target.hasWalkSteps()) {
		...
		if ((!player.clipedProjectile(target, maxDistance == 0 && !forceCheckClipAsRange(target))) || !Utils.isOnRange(player.getX(), player.getY(), size, target.getX(), target.getY(), target.getSize(), maxDistance)) {
			if (HUGGING_ENABLED && !player.hasWalkSteps()) {
				if (needCalc) {
					// TODO dont run right next to the player, run to the tile
					// next to the tree
					player.resetWalkSteps();
	...
	public static final int isRanging(Player player) {
		...
		if (name != null) { // those dont need arrows
			if (name.contains("knife") || name.contains("dart") || name.contains("javelin") || name.contains("thrownaxe") || name.contains("throwing axe") || name.contains("crystal bow") || name.equalsIgnoreCase("zaryte bow") || name.contains("chinchompa") || name.contains("Bolas") || name.contains("Toktz-xil-ul")) {
				return 2;
			}
		}
		int ammoId = player.getEquipment().getAmmoId();
	...
	public void addDelay(long foodDelay) {
		delay += foodDelay;
	}

	private static final boolean HUGGING_ENABLED = Boolean.TRUE;
}