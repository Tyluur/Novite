...
public class DTControler extends Controller {
	...
	public void process() {
		if (bosses != null) {
			...
			if (playerHp != this.playerHp || bossHp != this.bossHp) {
				this.playerHp = playerHp;
				this.bossHp = bossHp;
				refreshHPPercentages();
			}
		}
	}
	...
	public void refreshHPPercentages() {
		player.getPackets().sendIComponentText(1159, 40, player.getDisplayName() + " - " + playerHp + "% HP");
		player.getPackets().sendIComponentText(1159, 41, player.getDominionTower().getNextBoss().getName() + " - " + bossHp + "% HP");
	}

	@Override
	public void sendInterfaces() {
		if (bosses != null) {
			player.getInterfaceManager().sendTab(player.getInterfaceManager().hasResizableScreen() ? 10 : 8, 1159);
