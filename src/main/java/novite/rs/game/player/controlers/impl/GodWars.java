...
import novite.rs.game.player.actions.Smithing;
...
public class GodWars extends Controller {
	...
	public static final WorldTile[] GODS = { 
	...
	private static final WorldTile[] GOD_ROOMS = {
		new WorldTile(2864, 5354, 2), // BANDOS
		new WorldTile(2839, 5296, 2), // ARMADYL
		new WorldTile(2907, 5265, 0), // SARADOMIN
		new WorldTile(2925, 5331, 2), // ZAMORAK
	...
	public void sendInterfaces() {
		sendInterfaces(player.inArea(2916, 5317, 2937, 5332) ? 2 : inZamorakPrepare(player) ? 1 : 0);
	}
	...
	public boolean processObjectClick1(final WorldObject object) {
		if (object.getId() == 57225) {
			...
			player.setNextWorldTile(new WorldTile(2920, 5273, 0));
			return false;
			...
			if (!player.getInventory().contains(Smithing.HAMMER)) {
			...
			int requiredKc = !Constants.isVPS ? 0 : player.isDonator() ? 20 : 30;
			if (killCount[index] >= requiredKc) {
				...
				WorldTile tile = GOD_ROOMS[index];
				player.addWalkSteps(tile.getX(), tile.getY(), -1, false);
				player.sendMessage("You feel your body transfer through the door...");
				killCount[index] -= requiredKc;
				refresh();
			} else {
				player.getPackets().sendGameMessage("You need a killcount of " +requiredKc + " to enter the lair of this god.");
			}
			return true;
		}
		return true;
	...
	public void sendInterfaces(int interfaceIndex) {
		...
		if (!player.getInterfaceManager().containsInterface(INTERFACES[interfaceIndex])) {
			player.getInterfaceManager().sendTab(player.getInterfaceManager().hasResizableScreen() ? 10 : 8, INTERFACES[interfaceIndex]);
		}
		refresh();
	}

	public static boolean inZarosArea(Player player) {
		return player.inArea(2843, 5184, 2944, 5228);
	}
	...
	public void process() {
		if (lastInterfaceIndex != -1)
			sendInterfaces(lastInterfaceIndex);
	}

	public static boolean inArmadylPrepare(Player player) {
		return player.inArea(2866, 5268, 2874, 5273);
	}
