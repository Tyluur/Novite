...
import novite.rs.game.npc.others.YellowWizard;
import novite.rs.game.player.Player;
import novite.rs.game.player.actions.runecrafting.SiphionActionNodes;
import novite.rs.game.player.actions.runecrafting.SiphonActionCreatures;
import novite.rs.game.player.controlers.Controller;
import novite.rs.game.tasks.WorldTask;
import novite.rs.game.tasks.WorldTasksManager;
import novite.rs.utility.Utils;

public class RunespanControler extends Controller {

	public static WorldTile WIZARD_TOWER = new WorldTile(3106, 3162, 1);
	...
	public static WorldTile HIGHEST_LEVEL = new WorldTile(4296, 6038, 1);
	public static WorldTile VINE_LADDER = new WorldTile(3957, 6106, 1);

	private static final WorldTile[] RANDOM_LOCATIONS = { new WorldTile(3959, 6104, 1), new WorldTile(3996, 6055, 1), new WorldTile(3984, 6086, 1), new WorldTile(4399, 6041, 1)
	...
	private static int AIR_RUNE = 24215, EARTH_RUNE = 24216, WATER_RUNE = 24214, MIND_RUNE = 24217, FIRE_RUNE = 24213, ELEMENTAL_RUNE = -1, BODY_RUNE = 24218, RUNE_ESSENCE = 24227, CHAOS_RUNE = 24221, NATURE_RUNE = 24220, COSMIC_RUNE = 24223, ASTRAL_RUNE = 24224, LAW_RUNE = 24222, BLOOD_RUNE = 24225, DEATH_RUNE = 24219;
	...
	private static void removeRunes(Player player) {
		if (player.getFamiliar() != null && player.getFamiliar().getBob() != null) {
			for (Item item : player.getFamiliar().getBob().getBeastItems().getItems()) {
				if (item == null || item.getId() < 24213 || item.getId() > 24227) {
					continue;
				}
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (stage == 0) {
					...
					player.setNextForceMovement(new ForceMovement(player, 1, toTile, 5, Utils.getFaceDirection(toTile.getX() - object.getX(), toTile.getY() - object.getY())));
				} else if (stage == 1) {
					player.useStairs(-1, toTile, 1, 2);
	...
	public void start() {
		...
		player.getDialogueManager().startDialogue("SimpleMessage", "Welcome To the Runespan ", "WARNING:When you leave all the runes you make here will be removed and", "converted into reward points.");
	}
	...
	public void sendInterfaces() {
		player.getInterfaceManager().setOverlay(1274, false);
		player.getPackets().sendIComponentText(1274, 5, Integer.toString(player.getRuneSpanPoints()));
		refreshInventoryPoints();
	}

	private static final double[] REAL_POINTS_ORDER = { 0.5, 0.3, 0.1, 0.4, 0.2, 0.7, 2.5, 1.5, 1.1, 1.7, 0.9, 1.3, 3, 3.5, 0, 0 };

	public double getRunespanTotalInventoryPoints() {
		for (Item item : player.getInventory().getItems().getItems()) {
			if (item == null) {
				continue;
			}
			...
			double value = REAL_POINTS_ORDER[index];
			pointsValue += value;
		}
		return pointsValue;
	}
	...
	public int getIndex(int runeId) {
		int index = runeId - 24213;
		if (index < 0 || index > REAL_POINTS_ORDER.length) {
			return -1;
		}
		return index;
	}

	public void addRunespanPoints(int runeId) {
		int index = getIndex(runeId);
		if (index == -1) {
			return;
		}
		pointsValue += REAL_POINTS_ORDER[index];
		refreshInventoryPoints();
	}
	...
	public void refreshInventoryPoints() {
		player.getPackets().sendIComponentText(1274, 2, Integer.toString(getRuneSpanPoints()));
	}
	...
	public boolean processNPCClick1(NPC npc) {
		...
		switch (npc.getId()) {
			case 15402:
				...
				player.getInventory().addItem(new Item(24227, 25 - player.getInventory().getNumberOf(24227)));
				return false;
			case 15435:
			case 15434:
			case 15433:
			case 15432:
				player.getDialogueManager().startDialogue("WizardStudent");
				return false;
			case 15418:
				player.getDialogueManager().startDialogue("WizardFinix", 0, npc);
				return false;
		}
		return false;
	}
	...
	public boolean processObjectClick1(final WorldObject object) {
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {
						if (stage == 0) {
							...
							player.setNextForceMovement(new ForceMovement(player, 1, dest, 35, Utils.getFaceDirection(dest.getX() - object.getX(), dest.getY() - object.getY())));
		...
		if (object.getId() == 70508) {
			...
			WorldTasksManager.schedule(new WorldTask() {
				@Override
				public void run() {
					player.setNextAnimation(new Animation(16674));
				}
			}, 3);
			...
			player.useStairs(16675, HIGHEST_LEVEL, 2, 3);
			WorldTasksManager.schedule(new WorldTask() {
				@Override
				public void run() {
					player.setNextAnimation(new Animation(16674));
	...
	public YellowWizard getYellowWizard() {
		return yellowWizard;
	}
