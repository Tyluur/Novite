...
import novite.rs.utility.DTRank;
...
public final class DominionTower implements Serializable {
	...
	public static final class Boss {
		...
		public Boss(String name, String text, boolean forceMulti, Item item, int... ids) {
			this.name = name;
			this.text = text;
	...
	private static final Boss[] BOSSES = { new Boss("Elvarg", "Grrrr", 14548), new Boss("Corporeal Beast", null, true, null, 8133), new Boss("Delrith", "Grrrr", false, new Item(2402, 1), 14578), new Boss("Evil Chicken", "Bwak bwak bwak", 3375), new Boss("The Black Knight Titan", "Kill kill kill!", 14436), new Boss("Bouncer", "Grrr", 14483) };
		...
		CoresManager.slowExecutor.execute(new Runnable() {
			...
			public void run() {
				try {
					...
					RegionBuilder.copyAllPlanesMap(456, 768, mapBaseCoords[0], mapBaseCoords[1], 8);
	...
	public void startFight(final NPC[] bosses) {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (count == 0) {
					player.getInterfaceManager().sendTab(player.getInterfaceManager().hasResizableScreen() ? 9 : 8, 1172);
					...
					player.getPackets().sendIComponentText(1172, 6, BOSSES[nextBossIndex].name);
					...
					if (BOSSES[nextBossIndex].text != null) {
						bosses[0].setNextForceTalk(new ForceTalk(BOSSES[nextBossIndex].text));
					}
				} else if (count == 6) {
					...
					if (BOSSES[nextBossIndex].item != null) {
						World.addGroundItem(BOSSES[nextBossIndex].item, new WorldTile(getBaseX() + 26, getBaseY() + 33, 2));
					}
					player.closeInterfaces();
	...
	public NPC[] createBosses() {
		...
		for (int i = 0; i < BOSSES[nextBossIndex].ids.length; i++) {
			bosses[i] = World.spawnNPC(BOSSES[nextBossIndex].ids[i], new WorldTile(getBaseX() + 37 + (i * 2), getBaseY() + 31, 2), -1, true, true);
		}
		return bosses;
	}
	...
	public void openEnduranceMode() {
		...
		player.getPackets().sendIComponentText(1173, 20, BOSSES[nextBossIndex].name); // current
		player.getPackets().sendIComponentText(1173, 33, String.valueOf(progress + 1)); // current
		player.getPackets().sendIComponentText(1173, 47, "None. Good luck :o."); // current
		player.getPackets().sendIComponentText(1173, 24, String.valueOf(dominionFactor)); // current
		player.getPackets().sendIComponentText(1173, 26, dominionFactor == MAX_FACTOR ? "" : String.valueOf(getBossesTotalLevel() * 10)); // on win
		player.getPackets().sendIComponentText(1173, 28, String.valueOf(dominionFactor)); // on death
	}
