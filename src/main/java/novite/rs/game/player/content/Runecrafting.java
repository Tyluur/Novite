...
public final class Runecrafting {
	...
	public static void craftEssence(Player player, int rune, int level, double experience, boolean pureEssOnly, int... multipliers) {
		...
		if (player.getControllerManager().getController() instanceof GamesHandler) {
			...
			controller.addSkillPoints(runes / 5);
		}
	}
	...
	public static void locate(Player p, int xPos, int yPos) {
		...
		p.getPackets().sendGameMessage("The talisman pulls towards " + y + "-" + x + ".");
	}
	...
	public enum Talismans {
		...
		AIR(1438, 2841, 4829, 0), EARTH(1440, 2655, 4830, 0), FIRE(1442, 2577, 4846, 0), WATER(1444, 3493, 4832, 0), BODY(1446, 2520, 4833, 0), MIND(1448, 2792, 4834, 0), BLOOD(1450, 2468, 4889, 1), CHAOS(1452, 2281, 4837, 0), COSMIC(1454, 2162, 4833, 0), DEATH(1456, 2208, 4830, 0, 0), LAW(1458, 2464, 4818, 0), NATURE(1462, 2400, 4835, 0);

		private final WorldTile tile;
		private final int id;

		Talismans(int id, int... tiles) {
			this.id = id;
			this.tile = new WorldTile(tiles[0], tiles[1], tiles[2]);
		}
		...
		/**
		 * Gets the talisman by the talisman id
		 * 
		 * @param id
		 *            The id
		 * @return
		 */
		public static Talismans getTalisman(int id) {
			for (Talismans talisman : Talismans.values()) {
				if (talisman.id == id)
					return talisman;
			}
			return null;
