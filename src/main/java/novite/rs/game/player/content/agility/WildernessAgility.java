...
import novite.rs.game.player.content.achievements.impl.WildernessAgilityHardAchievement;
...
public class WildernessAgility {
	...
	public static void swingOnRopeSwing(final Player player, WorldObject object) {
		if (!Agility.hasLevel(player, 52)) {
			return;
		} else if (player.getY() != 3953 && player.getY() != 3958) {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (getStage(player) != 4) {
					...
					player.getAchievementManager().notifyUpdate(WildernessAgilityHardAchievement.class);
					setStage(player, 0);
				}
			}
		}, 8);
