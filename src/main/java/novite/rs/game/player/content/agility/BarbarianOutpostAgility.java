...
import novite.rs.game.player.content.achievements.impl.BarbarianMediumAchievement;
...
public class BarbarianOutpostAgility {
	...
	public static void swingOnRopeSwing(final Player player, WorldObject object) {
		...
		final WorldTile toTile = new WorldTile(object.getX(), 3549, player.getPlane());
	...
	private static void walkAcrossLogBalanceEnd(final Player player, WorldObject object) {
		...
		final WorldTile toTile = new WorldTile(2541, object.getY(), player.getPlane());
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (stage == 3) {
					...
					player.getAchievementManager().notifyUpdate(BarbarianMediumAchievement.class);
				}
			}
