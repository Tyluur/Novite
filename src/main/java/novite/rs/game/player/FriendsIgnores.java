...
public class FriendsIgnores implements Serializable {
	...
	private ArrayList<String> tillLogoutIgnores;
	...
	public void sendMessage(Player p2, String message) {
		...
		FileLogger.getFileLogger().writeLog("pms/", player.getDisplayName() + " sent " + p2.getDisplayName() + ":\t" + message, true);
	}
	...
	private ArrayList<String> fixList(ArrayList<String> list) {
		Iterator<String> it = list.iterator();
		while(it.hasNext()) {
			String e = it.next();
			if (Utils.containsInvalidCharacter(e)) {
				it.remove();
			}
		}
		return list;
	...
	public void addIgnore(String username, boolean tillLogout) {
		...
		player.getPackets().sendIgnore(Utils.formatPlayerNameForDisplay(p2 != null ? p2.getUsername() : username), p2 != null ? p2.getDisplayName() : Utils.formatPlayerNameForDisplay(username));
	}

	public boolean containsIgnore(String username) {
		username = Utils.formatPlayerNameForProtocol(username);
		return ignores.contains(username) || tillLogoutIgnores.contains(username);
	}
	...
	public void init() {
		...
		for (String username : friends) {
			...
			player.getPackets().sendFriend(Utils.formatPlayerNameForDisplay(username), displayName, 1, p2 != null && isOnline(p2), false);
		}
		ignores = fixList(ignores);
		for (String username : ignores) {
			...
			player.getPackets().sendIgnore(displayName, Utils.formatPlayerNameForDisplay(username));
		}
		if (privateStatus != 2) {
			sendFriendsMyStatus(true);
		}
