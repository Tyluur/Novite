...
public class Prayer implements Serializable {
	...
	private boolean[][] unlockedPrayers;
	...
	public int getQuickPrayerAmount() {
		int amount = 0;
	...
	public void switchQuickPrayers() {
		if (onPrayersCount == 0 && getQuickPrayerAmount() == 0) {
	...
	private boolean usePrayer(int prayerId) {
		...
		if (player.getControllerManager().getController() instanceof DuelArena && player.getLastDuelRules().getRule(5) && ((DuelArena) player.getControllerManager().getController()).isDueling()) {
			player.sendMessage("You cannot activate prayers in this duel.");
			return false;
		}
		if (!player.isDonator() && !getUnlockedPrayers()[getPrayerBook()][prayerId]) {
			player.getDialogueManager().startDialogue(SimpleItemMessage.class, Scrolls.RIGOUR_SCROLL, "You have not yet unlocked this prayer.", "You must buy the scroll to unlock it!");
			return false;
		}
		if (player.getCurrentFriendChat() != null) {
			ClanWars war = player.getCurrentFriendChat().getClanWars();
	...
	public void processPrayerDrain() {
		...
		int drain = 0;
		int bonus = player.getCombatDefinitions().getBonuses()[CombatDefinitions.PRAYER_BONUS];
		for (int index = 0; index < onPrayers[book].length; index++) {
			if (onPrayers[book][index]) {
				long drainTimer = nextDrain[index];
				if (drainTimer != 0 && drainTimer <= currentTime) {
					int rate = (int) ((prayerDrainRate[getPrayerBook()][index] * 1000) + (bonus * 50));
	...
	public Prayer() {
		...
		setUnlockedPrayers(new boolean[][] { new boolean[30], new boolean[20] });
		for (int i = 0; i < getUnlockedPrayers().length; i++) {
			for (int j = 0; j < getUnlockedPrayers()[i].length; j++) {
				if (i == 0 && j >= 28 || i == 1 && j >= 18)
					continue;
				getUnlockedPrayers()[i][j] = true;
			}
		}
		prayerpoints = 10;
	}
	...
	/**
	 * @param unlockedPrayers the unlockedPrayers to set
	 */
	public void setUnlockedPrayers(boolean[][] unlockedPrayers) {
		this.unlockedPrayers = unlockedPrayers;
	}
