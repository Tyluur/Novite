...
public final class LocalPlayerUpdate {
	...
	public LocalPlayerUpdate(Player player) {
		...
		localPlayersIndexes = new int[Constants.PLAYERS_LIMIT];
		outPlayersIndexes = new int[2048];
		regionHashes = new int[2048];
		cachedAppearencesHashes = new byte[Constants.PLAYERS_LIMIT][];
	}
	...
	private boolean needsAdd(Player p) {
		return p != null && !p.hasFinished() && player.withinDistance(p, 14) && localAddedPlayers < MAX_PLAYER_ADD;
	}
	...
	private void processOutsidePlayers(OutputStream stream, OutputStream updateBlockData) {
		...
		stream.finishBitAccess();
		//System.out.println("GPI took " + (System.nanoTime() - start) + " for " + player.getUsername());
	}
	...
	public OutputStream createPacketAndProcess() {
		...
		stream.writePacketVarShort(player, 69);
		...
		//System.out.println("LocalPlayerUpdate took " + (System.nanoTime() - start) + " nano seconds for " + player.getUsername());
		return stream;
	}
