	...
	public boolean degradeCompletly(Item item) {
		...
		try {
			if (ItemConstants.itemDegradesWhileWearing(item.getId()) || ItemConstants.itemDegradesWhileCombating(item.getId())) {
				...
				int newId = -1;
				if (Bob.isBarrows(item.getId()))
					newId = Bob.getDegraded(true, item.getId());
				else
	...
	private void degrade(int itemId, int defaultCharges, int slot) {
		...
		if (c == null)
			...
			if (c <= 0) {
				...
				if (Bob.isBarrows(itemId) && Bob.getCharges(itemId) == 0) {
					int amt = player.getEquipment().getItems().getNumberOf(itemId);
					player.getEquipment().deleteItem(itemId, amt);
					player.getInventory().addDroppable(new Item(itemId, amt));
				} else {
					player.getEquipment().getItems().set(slot, newId != -1 ? new Item(newId, 1) : null);
				}
				if (newId == -1) {
					if (!Bob.isBarrows(itemId)) {
						player.getPackets().sendGameMessage("Your " + ItemDefinitions.getItemDefinitions(itemId).getName() + " turns into dust.");
					}
				} else {
		...
		//System.out.println(ItemDefinitions.getItemDefinitions(itemId).name + " [" + itemId + ", " + c + "]");
