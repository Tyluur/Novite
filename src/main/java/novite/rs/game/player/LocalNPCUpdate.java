...
public final class LocalNPCUpdate {
	...
	private void processInScreenNPCs(OutputStream stream, OutputStream updateBlockData) {
		...
		for (Iterator<NPC> it = localNPCs.iterator(); it.hasNext();) {
			...
			boolean needUpdate = (updateBlockData.getOffset() + stream.getOffset() < (Constants.PACKET_SIZE_LIMIT - 500)) && n.needMasksUpdate();
	...
	private void addInScreenNPCs(OutputStream stream, OutputStream updateBlockData) {
		for (int regionId : player.getMapRegionsIds()) {
			...
			for (int npcIndex : indexes) {
				if (localNPCs.size() == Constants.LOCAL_NPCS_LIMIT || (updateBlockData.getOffset() + stream.getOffset() > (Constants.PACKET_SIZE_LIMIT - 500))) {
					break;
				}
				...
				stream.writeBits(3, ((NPCAutoSpawn) JsonHandler.getJsonLoader(NPCAutoSpawn.class)).getDirection(n).getValue());
