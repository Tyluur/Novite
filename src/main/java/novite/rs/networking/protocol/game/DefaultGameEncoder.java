...
import novite.rs.game.DynamicRegion;
...
import novite.rs.game.player.HintIcon;
...
public class DefaultGameEncoder extends Encoder {
	...
	public void sendInputLongTextScript(String message, StringInputAction onInputGivenAction) {
		player.getTemporaryAttributtes().put("long_string_input_action", onInputGivenAction);
	...
	public void sendRunScriptpz(int id, Object[] params, String types) {
		...
		packet.writePacketVarShort(player, 70);
	...
	public void sendDynamicMapRegion(boolean wasAtDynamicRegion) {
		...
		int sceneLength = Constants.MAP_SIZES[player.getMapSize()] >> 4;
		...
		for (int index = 0; index < newRegionIdsCount; index++) {
			int[] xteas = MapXTEA.getKey(regionIds[index]);
			if (xteas == null) {
				xteas = new int[4];
			}
	...
	public void sendInterface(boolean nocliped, int windowId, int windowComponentId, int interfaceId) {
		if (windowId != 752 || windowComponentId != 9 && windowComponentId != 12) {
			...
			if (!player.getInterfaceManager().addInterface(windowId, windowComponentId, interfaceId)) {
				System.err.println(new StringBuilder("Error adding interface: ").append(windowId).append(" , ").append(windowComponentId).append(" , ").append(interfaceId).toString());
				return;
			}
		}
		OutputStream stream = new OutputStream(8);
	...
	public void sendLogout() {
		...
		if (future != null) {
			...
			System.out.println("Sent logout packet & closed session");
		}
	}
	...
	public void receiveClanChatQuickMessage(boolean myClan, String display, int rights, QuickChatMessage message) {
		OutputStream stream = new OutputStream();
		stream.writePacketVarByte(player, 131);
		stream.writeByte(myClan ? 1 : 0);
		stream.writeString(display);
		for (int i = 0; i < 5; i++) {
			stream.writeByte(Utils.getRandom(255));
