...
import novite.rs.cache.scripts.CacheFixer;
...
public final class ItemDefinitions {
	...
	public static ItemDefinitions getItemDefinition(Store cache, int itemId) {
		return new ItemDefinitions(cache, itemId);
	}

	public ItemDefinitions(Store store, int id) {
		this.id = id;
	...
	public byte[] encode() {
		...
		boolean isTradeable = CacheFixer.isTradeable(id);
		stream.writeByte(15);
		stream.writeByte(isTradeable ? 1 : 0);

		if (membersOnly && certTemplateId == -1) {
			stream.writeByte(16);
		}

		boolean isExchangeable = ItemConstants.shouldExchange(id);
		//System.out.println(name + " is " + (isExchangeable ? "" : "un") + "exchangeable.");
		stream.writeByte(17);
	...
	/**
	 * @param showInGrandExchange the showInGrandExchange to set
	 */
