...
public final class Store {
	...
	public Store(String path, boolean newProtocol, int[][] keys)
		...
		index255 = new MainFile(255, data, new RandomAccessFile(path
				+ "main_file_cache.idx255", "rw"), readCachedBuffer,
				newProtocol);
		int idxsCount = index255.getArchivesCount();
		indexes = new Index[idxsCount];
		for (int id = 0; id < idxsCount; id++) {
			Index index = new Index(index255, new MainFile(id, data,
					new RandomAccessFile(path + "main_file_cache.idx" + id,
							"rw"), readCachedBuffer, newProtocol),
					keys == null ? null : keys[id]);
	...
	public final byte[] generateIndex255Archive255Current(
			BigInteger grab_server_private_exponent,
			BigInteger grab_server_modulus) {
		...
		if (grab_server_private_exponent != null && grab_server_modulus != null)
			hash = Utils.cryptRSA(hash, grab_server_private_exponent,
					grab_server_modulus);
	...
	public byte[] generateIndex255Archive255() {
		return Constants.CLIENT_BUILD < 614 ? generateIndex255Archive255Outdated()
				: generateIndex255Archive255Current(null, null);
	}
	 ...
	 * returns index
	 */
	public int addIndex(boolean named, boolean usesWhirpool,
			int tableCompression) throws IOException {
	...
	public void resetIndex(int id, boolean named, boolean usesWhirpool,
			int tableCompression) throws FileNotFoundException, IOException {
		resetIndex(id, indexes, named, usesWhirpool, tableCompression);
	}

	public void resetIndex(int id, Index[] indexes, boolean named,
			boolean usesWhirpool, int tableCompression)
		...
		indexes[id] = new Index(index255, new MainFile(id, data,
				new RandomAccessFile(path + "main_file_cache.idx" + id, "rw"),
