...
public final class ReferenceTable {
	...
	public void addEmptyArchiveReference(int archiveId) {
		...
		int[] newValidArchiveIds = Arrays.copyOf(validArchiveIds,
				validArchiveIds.length + 1);
		...
		if (archives.length <= archiveId) {
			ArchiveReference[] newArchives = Arrays.copyOf(archives,
					archiveId + 1);
	...
	public Object[] encodeHeader(MainFile mainFile) {
		...
		if (usesWhirpool)
			for (int index = 0; index < validArchiveIds.length; index++)
				stream.writeBytes(archives[validArchiveIds[index]]
						.getWhirpool());
		for (int index = 0; index < validArchiveIds.length; index++)
		...
		if (named) {
			for (int index = 0; index < validArchiveIds.length; index++) {
				ArchiveReference archive = archives[validArchiveIds[index]];
				for (int index2 = 0; index2 < archive.getValidFileIds().length; index2++)
					stream.writeInt(archive.getFiles()[archive
							.getValidFileIds()[index2]].getNameHash());
			}
		}
	...
	private void decodeHeader() {
		...
		int validArchivesCount = protocol >= 7 ? stream.readBigSmart() : stream
				.readUnsignedShort();
		validArchiveIds = new int[validArchivesCount];
		int lastArchiveId = 0;
		int biggestArchiveId = 0;
		for (int index = 0; index < validArchivesCount; index++) {
			int archiveId = lastArchiveId += protocol >= 7 ? stream
					.readBigSmart() : stream.readUnsignedShort();
		...
		for (int index = 0; index < validArchivesCount; index++) {
			...
			ArchiveReference archive = archives[validArchiveIds[index]];
			for (int index2 = 0; index2 < archive.getValidFileIds().length; index2++) {
				int fileId = lastFileId += protocol >= 7 ? stream
						.readBigSmart() : stream.readUnsignedShort();
		...
		if (named) {
			for (int index = 0; index < validArchivesCount; index++) {
				ArchiveReference archive = archives[validArchiveIds[index]];
				for (int index2 = 0; index2 < archive.getValidFileIds().length; index2++)
					archive.getFiles()[archive.getValidFileIds()[index2]]
